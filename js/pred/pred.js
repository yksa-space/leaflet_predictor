...
function populateFormByUUID(pred_uuid) {
  $.get(
    "ajax.php",
    { action: "getModelByUUID", uuid: pred_uuid },
    function (data) {
      if (!data.valid) {
...
function handlePred(pred_uuid) {
  ...
  firstJSONProgressHandle = setTimeout(
    "firstJSONProgress('" + pred_uuid + "')",
    firstAjaxDelay
  );
}
  ...
  $.get(
    ...
    function (data) {
      if (data != null) {
        ...
        throwError(
          "Sorry, we couldn't find the data for that UUID. " +
...
function processCompletedPrediction(progress) {
  ...
  getCSV(current_uuid);
  appendDebug(
    "Server gave a prediction run timestamp of " + progress["run_time"]
  );
  appendDebug(
    "Server said it used the " + progress["dataset"] + " GFS Dataset"
  );

  var warnings =
    "<b>The prediction completed, but with warnings!<br>" +
...
function processProgress(progress) {
  if (progress["error"]) {
    clearInterval(ajaxEventHandle);
    appendDebug(
      "There was an error in running the prediction: " + progress["error"]
    );
    resetGUI();
    toggleWindow(
      "scenario_template",
    ...
    if (progress["pred_complete"] == true) {
      // pred has finished
...
function parseCSV(lines) {
  ...
  var launch_icon = new google.maps.MarkerImage(
    launch_img,
  ...
  var land_icon = new google.maps.MarkerImage(
    land_img,
    new google.maps.Size(10, 10),
    new google.maps.Point(0, 0),
    new google.maps.Point(5, 5)
  ...
  var burst_icon = new google.maps.MarkerImage(
    burst_img,
